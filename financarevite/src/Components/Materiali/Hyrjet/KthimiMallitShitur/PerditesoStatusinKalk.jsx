      ...
      await axios
        ...
        .then(async (teDhenat) => {
          for (let p of teDhenat.data) {
            await axios.get(
              `${API_BASE_URL}/api/Faturat/hapAsgjesiminKthimin/perditesoStokunQmimin?idKalkulimi=${p.idRegjistrimit}&idProdukti=${p.idProduktit}&idTeDhenatKalkulimit=${p.id}&lloji=KMSH`,
              authentikimi
            );
          }
        });
