      ...
      await axios
        ...
        .then(async (teDhenat) => {
          for (let p of teDhenat.data) {
            await axios.get(
              `https://localhost:7285/api/Faturat/hapAsgjesiminKthimin/perditesoStokunQmimin?idKalkulimi=${p.idRegjistrimit}&idProdukti=${p.idProduktit}&idTeDhenatKalkulimit=${p.id}&lloji=KMSH`,
              authentikimi
            );
          }
        });
